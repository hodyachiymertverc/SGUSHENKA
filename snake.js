...
const SNAKE_FOOD_SRC = 'img/sgushenka.png';
let snakeFoodImg = null;
(function preloadSnakeFoodImg(){
  const img = new Image();
  img.onload = ()=> { snakeFoodImg = img; };
  img.onerror = ()=> {};
  img.src = SNAKE_FOOD_SRC;
})();
...
const Snake = {
  ...
  /* ---------------- рекорды ---------------- */
  collectionFor(diff){ return diff === 'hard' ? 'snakeRecordsHard' : 'snakeRecordsEasy'; },
  ...
  fitCanvas(){
    ...
    const hud = document.getElementById('snakeHud');
  ...
  render(){
    ...
    if(this.food){
      ...
      if(snakeFoodImg){
        const pad = cs * 0.12;
        ctx.drawImage(snakeFoodImg, fx + pad, fy + pad, cs - pad * 2, cs - pad * 2);
      } else {
    ...
    this.snakeBody.forEach((seg, i)=>{
      ...
      snakeRoundRect(ctx, sx + cs * 0.06, sy + cs * 0.06, cs * 0.88, cs * 0.88, cs * 0.28);
      ctx.fill();
      ctx.strokeStyle = '#5E3A1A';
  ...
  bindUI(){
    ...
    document.querySelectorAll('[data-snaketab]').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        const tab = btn.dataset.snaketab;
        snakeHide(document.getElementById('snakeRecordsListEasy'));
        snakeHide(document.getElementById('snakeRecordsListHard'));
        snakeShow(document.getElementById(tab === 'hard' ? 'snakeRecordsListHard' : 'snakeRecordsListEasy'));
      });
    });
