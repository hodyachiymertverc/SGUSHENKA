/* =========================================================
   ⚠️ НЕ СОБИРАЕТСЯ. Этот файл сохранён только для истории и
   НЕ входит в сборку (см. "include" в tsconfig.json — там
   явно указан только ts/snake.ts). Причина: этот файл — старая,
   урезанная версия игры (нет режима «Камикадзе», нет выбора
   загрузки, нет таблиц рекордов по дрону), а актуальная,
   полнофункциональная версия — только в js/drone.js, она
   написана и поддерживается напрямую в JS, а не генерируется
   отсюда. Если когда-нибудь понадобится снова собирать дрон из
   TypeScript — сначала нужно перенести сюда ВСЕ фичи из
   js/drone.js (бомбы/камикадзе/лидерборды), иначе сборка (в т.ч.
   автоматическая на каждом push, см. .github/workflows/
   deploy-pages.yml) перезапишет рабочий js/drone.js этим же
   устаревшим кодом и сломает игру.
========================================================= */
/* =========================================================
