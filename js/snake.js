/* Этот файл СГЕНЕРИРОВАН автоматически из ts/snake.ts командой
   `npm run build` (см. package.json, tsconfig.json) — редактировать
   его руками не нужно, правки нужно вносить в ts/snake.ts, а сюда
   они попадут при следующей сборке (в т.ч. автоматически при
   деплое — см. .github/workflows/deploy-pages.yml). */
"use strict";
/* =========================================================
...
function hasCloudDB() { return !!window.DB; }
function hasCloudDefaults() { return !!window.DEFAULTS; }
function escapeHtmlS(str) {
    const d = document.createElement('div');
    d.textContent = str == null ? '' : str;
    return d.innerHTML;
}
function snakeShow(el) { if (el)
    el.classList.remove('hidden'); }
function snakeHide(el) { if (el)
    el.classList.add('hidden'); }
function snakeShowEl(el, show) { if (el)
...
function snakeDist(x1, y1, x2, y2) {
    const dx = x2 - x1, dy = y2 - y1;
    return Math.sqrt(dx * dx + dy * dy);
}
function snakeClamp(v, min, max) { return Math.max(min, Math.min(max, v)); }
function snakeNormAngle(a) { a = (a + Math.PI) % (Math.PI * 2); if (a < 0)
    a += Math.PI * 2; return a - Math.PI; }
...
const Snake = {
    ...
    init() {
        ...
        DB.watchItem('snakePlayers', this.playerId, doc => {
            this.data = Object.assign({ totalCaught: 0, gamesPlayed: 0, bestEasy: 0, bestMedium: 0, bestHard: 0, bestOnline: 0, unlocked: {}, selectedSkin: { type: 'random' }, customSkinColors: [] }, doc || {});
            this._ready = true;
            this.renderMenuUI();
            this.renderProfileUI();
            this.renderSkinModal();
    ...
    checkAchievements() {
        ...
        const metric = (type) => {
            if (type === 'caught')
                return this.data.totalCaught || 0;
            if (type === 'games')
                return this.data.gamesPlayed || 0;
            if (type === 'bestEasy')
                return this.data.bestEasy || 0;
            if (type === 'bestMedium')
                return this.data.bestMedium || 0;
            if (type === 'bestHard')
                return this.data.bestHard || 0;
            if (type === 'bestOnline')
                return this.data.bestOnline || 0;
            return 0;
        };
    ...
    renderSkinColorGrid() {
      ...
      ${customUnlocked ? '' : '<div class="skin-lock-note">Открой все скины ниже</div>'}`;
        ...
        if (editor)
            snakeShowEl(editor, sel.type === 'custom' && customUnlocked);
        if (sel.type === 'custom' && customUnlocked)
    ...
    selectSkin(sel) {
        ...
        if (hasCloudDB())
            DB.setItem('snakePlayers', this.playerId, { selectedSkin: sel });
        this.renderSkinModal();
    },
    addCustomColorInput() {
        ...
        if (wrap.children.length >= 3)
            return; // максимум 3 цвета в кастомном скине
    ...
    saveCustomSkin() {
        ...
        if (hasCloudDB())
            DB.setItem('snakePlayers', this.playerId, { customSkinColors: colors, selectedSkin: { type: 'custom' } });
        this.renderSkinModal();
    },
    ...
    resolvePlayerSkin() {
        ...
        if (sel.type === 'color') {
            ...
            if (skin && this.isSkinUnlocked(skin))
                return { type: 'color', colors: skin.colors || [] };
        }
        if (sel.type === 'image') {
    ...
    renderMenuUI() {
        ...
        if (emojiEl)
            emojiEl.textContent = level ? level.emoji : '🥄';
        const nameEl = document.getElementById('snakeLevelName');
        if (nameEl)
            nameEl.textContent = level ? level.name : 'Новичок-змейка';
    ...
    skinColorsFor(s) {
        if (s && s.skin && 'colors' in s.skin && Array.isArray(s.skin.colors) && s.skin.colors.length)
    ...
    bodyHit(px, py, trail, radius) {
        ...
        for (let i = 0; i < trail.length; i += stride) {
            if (snakeDist(px, py, trail[i].x, trail[i].y) < radius)
                return true;
        }
        return false;
    },
    ...
    startGame(diffKey) {
        ...
        const labelMap = { easy: '🥄 Лёгкий', medium: '🐍 Средний', hard: '💣 Сложный', online: '🌐 Онлайн' };
        if (diffPill)
            diffPill.textContent = labelMap[diffKey] || '🥄 Лёгкий';
        this.fitCanvas();
    ...
    resetWorld(diffKey) {
        ...
        for (let i = 0; i < preset.count; i++)
            this.bots.push(this.makeBot(diffKey));
        this.peers = {};
    },
    beginLoop() {
        this.running = true;
        ...
        this._lbAcc = 0;
        if (!this._loopBound)
            this._loopBound = (now) => this.loop(now);
    ...
    openOnlineModal() {
        ...
        if (!hasCloudDB()) {
            if (statusEl)
                statusEl.textContent = 'Онлайн работает только с настроенным облаком (см. документацию администратора) — пока можно поиграть только против ботов.';
            if (spinner)
                snakeHide(spinner);
            return;
        }
        if (statusEl)
            statusEl.textContent = 'Ищем свободную комнату…';
        if (spinner)
            snakeShow(spinner);
        this.watchOpenRooms();
        this.matchmake();
    ...
    matchmake() {
        ...
        DB.listOnce('snakeRooms').then(async (rooms) => {
            ...
            const statusEl = document.getElementById('snakeOnlineStatusText');
            if (statusEl)
                statusEl.textContent = 'Не удалось подключиться к онлайну. Попробуй ещё раз.';
        });
    },
    joinRoomByCode() {
        ...
        DB.getItemOnce('snakeRooms', code).then(doc => {
            ...
            if (!doc) {
                if (errEl) {
                    errEl.textContent = `Комната «${code}» не найдена — проверь код.`;
                    snakeShow(errEl);
                }
                return;
            }
            ...
            if (errEl) {
                errEl.textContent = 'Не удалось проверить код. Попробуй ещё раз.';
                snakeShow(errEl);
            }
        });
    },
    registerInRoom(code) {
        ...
        const statusEl = document.getElementById('snakeOnlineStatusText');
        if (statusEl)
            statusEl.textContent = 'Комната готова — можно начинать! Поделись кодом, чтобы позвать друга.';
        const spinner = document.getElementById('snakeOnlineSpinner');
        if (spinner)
            snakeHide(spinner);
        if (!hasCloudDB())
    ...
    handleOnlinePlayersUpdate(list) {
        ...
        (list || []).forEach(doc => {
            ...
            if (now - (doc.ts || 0) > 12000) {
                if (hasCloudDB())
                    DB.deleteItem('snakeOnlinePlayers', doc.id);
                return;
            }
            if (doc.roomId !== this.roomId)
                return;
            countInRoom++;
            if (doc.id === this.playerId)
                return;
            activeIds.add(doc.id);
            let peer = this.peers[doc.id];
            const isNew = !peer;
            if (!peer) {
                peer = this.peers[doc.id] = { trail: [] };
                peer.id = doc.id;
            }
            ...
            if (isNew) {
                peer.x = doc.x;
                peer.y = doc.y;
                peer.angle = doc.angle || 0;
                peer.trail = [{ x: doc.x, y: doc.y }];
                peer.maxSpanEver = (doc.len || SNAKE_BASE_LEN) + 60;
            }
        });
        Object.keys(this.peers).forEach(id => { if (!activeIds.has(id))
            delete this.peers[id]; });
        const countEl = document.getElementById('snakeOnlineRoomCount');
        if (countEl)
            countEl.textContent = String(Math.max(1, countInRoom));
    },
    ...
    updatePeersMotion(dt) {
        ...
        this.peerList().forEach(peer => {
            ...
            if (cut < peer.trail.length)
                peer.trail.length = cut;
        });
    },
    cancelOnlineSearch() {
    ...
    startOnlineGame() {
        ...
        const diffPill = document.getElementById('snakeDiffPill');
        if (diffPill)
            diffPill.textContent = '🌐 Онлайн · ' + this.roomId;
        ...
        if (hasCloudDB() && this.roomId) {
            DB.setItem('snakeOnlinePlayers', this.playerId, {
                name: getNickname(), color: this.colorFor(this.playerId), skin: this.player.skin
            });
        }
        this.desiredAngle = this.player.angle;
        this.inputBoost = false;
        ...
        this.bots = [];
        for (let i = 0; i < preset.count; i++)
            this.bots.push(this.makeBot('online'));
        this._onlineSyncAcc = 0;
        this.beginLoop();
    },
    updateOnlineSync(dt) {
        this._onlineSyncAcc = (this._onlineSyncAcc || 0) + dt;
        if (this._onlineSyncAcc < SNAKE_ONLINE_SYNC_INTERVAL)
            return;
        this._onlineSyncAcc = 0;
        if (!hasCloudDB() || !this.roomId || !this.player)
            return;
        DB.setItem('snakeOnlinePlayers', this.playerId, {
    ...
    fitCanvas() {
        ...
        if (!screen || !canvas || !wrap || screen.classList.contains('hidden'))
            return;
    ...
    loop(now) {
        ...
        if (this.running && this._loopBound)
            requestAnimationFrame(this._loopBound);
    },
    update(dt) {
        if (!this.player)
    ...
    stepSnake(s, dt) {
        ...
        if (s.visLen === undefined)
            s.visLen = s.len;
        const lenDiff = s.len - s.visLen;
        if (Math.abs(lenDiff) > 0.05)
            s.visLen += lenDiff * Math.min(1, dt * 6);
        else
            s.visLen = s.len;
    ...
    botLookaheadDanger(b, allObstacles) {
        ...
        const hitsObstacle = (angle) => {
            for (const dist of probeDists) {
                ...
                for (const trail of trails) {
                    ...
                    for (let i = 0; i < trail.length; i += stride) {
                        if (snakeDist(px, py, trail[i].x, trail[i].y) < dangerR)
                            return true;
                    }
                }
            }
        ...
        if (!hitsObstacle(b.angle))
            return null; // курс впереди свободен
        const offsets = [0.5, -0.5, 0.9, -0.9, 1.4, -1.4, 2.2, -2.2, Math.PI];
        for (const off of offsets) {
            const angle = b.angle + off;
            if (!hitsObstacle(angle))
                return { escapeAngle: angle };
        }
    ...
    updateFood() {
        while (this.food.filter(f => !f.big).length < SNAKE_SMALL_FOOD_COUNT)
            this.food.push(this.randomFood(false));
        while (this.food.filter(f => f.big).length < SNAKE_BIG_FOOD_COUNT)
            this.food.push(this.randomFood(true));
        if (this.player)
    ...
    checkCollisions() {
        ...
        this.bots.forEach(b => { if (b.alive)
            botTrails.set(b, this.visibleSpanTrail(b)); });
        ...
        for (const b of this.bots) {
            ...
            if (this.bodyHit(p.x, p.y, botTrails.get(b), rb)) {
                this.killSnake(p, 'enemy');
                return;
            }
            const rp = this.collisionRadius(p);
            if (this.bodyHit(b.x, b.y, playerTrailForBots, rp)) {
                this.killSnake(b, 'enemy');
            }
        }
        ...
        for (let i = 0; i < this.bots.length; i++) {
            ...
            for (let j = 0; j < this.bots.length; j++) {
                ...
                if (!bb.alive)
                    continue;
                if (this.bodyHit(a.x, a.y, botTrails.get(bb), this.collisionRadius(bb))) {
                    this.killSnake(a, 'enemy');
                    break;
                }
            }
        }
        ...
        if (this.mode === 'online') {
            for (const id in this.peers) {
                ...
                if (this.bodyHit(p.x, p.y, trail, rp)) {
                    this.killSnake(p, 'enemy');
                    return;
                }
            }
    ...
    endGame(reason) {
        ...
        if (titleEl)
            titleEl.textContent = titleMap[reason] || '💥 Игра окончена!';
    ...
    drawFood(ctx, f) {
        ...
        if (f.big) {
            ...
            ctx.fillStyle = 'rgba(255,159,69,0.25)';
            ctx.fill();
        }
        if (img) {
    ...
    drawSnakeBody(ctx, s) {
        ...
        if (pts.length < 2)
            pts.push({ x: s.x - Math.cos(s.angle) * 4, y: s.y - Math.sin(s.angle) * 4 });
    ...
    getSkinCircleSprite(src, diameter) {
        ...
        if (!img || !img.complete || !img.naturalWidth)
            return null;
        if (!this._skinCircleCache)
            this._skinCircleCache = {};
    ...
    drawSnakeBandBody(ctx, s, pts, r) {
        ...
        for (let i = 1; i < pts.length; i++)
            outline.lineTo(pts[i].x, pts[i].y);
        ctx.lineJoin = 'round';
        ctx.lineCap = 'round';
    ...
    drawNicknames(ctx, camX, camY, w, h, zoom) {
        ...
        list.forEach(s => {
            ...
            if (sx < -60 || sx > w + 60 || sy < -30 || sy > h + 30)
                return;
    ...
    renderLeaderboard() {
        if (!this.player)
            return;
        const entries = [{ id: 'me', name: 'Вы', score: this.score || 0, isMe: true }];
    ...
    bindUI() {
        ...
        if (onlineBtn)
            onlineBtn.addEventListener('click', () => this.openOnlineModal());
        const onlineCancel1 = document.getElementById('snakeOnlineCancelBtn');
        if (onlineCancel1)
            onlineCancel1.addEventListener('click', () => this.cancelOnlineSearch());
        const onlineCancel2 = document.getElementById('snakeOnlineCancelBtn2');
        if (onlineCancel2)
            onlineCancel2.addEventListener('click', () => this.cancelOnlineSearch());
        const onlineJoinBtn = document.getElementById('snakeOnlineJoinBtn');
        if (onlineJoinBtn)
            onlineJoinBtn.addEventListener('click', () => this.joinRoomByCode());
        const onlineStartBtn = document.getElementById('snakeOnlineStartBtn');
        if (onlineStartBtn)
            onlineStartBtn.addEventListener('click', () => this.startOnlineGame());
        const skinBtn = document.getElementById('snakeSkinBtn');
        if (skinBtn)
            skinBtn.addEventListener('click', () => this.openSkinModal());
        const skinCloseBtn = document.getElementById('snakeSkinCloseBtn');
        if (skinCloseBtn)
            skinCloseBtn.addEventListener('click', () => snakeHide(document.getElementById('snakeSkinModal')));
        ...
        if (customAddBtn)
            customAddBtn.addEventListener('click', () => this.addCustomColorInput());
        const customSaveBtn = document.getElementById('snakeCustomColorSaveBtn');
        if (customSaveBtn)
            customSaveBtn.addEventListener('click', () => this.saveCustomSkin());
        ...
        if (exitBtn)
            exitBtn.addEventListener('click', () => this.goToMenu());
        const pauseExitBtn = document.getElementById('snakePauseExitBtn');
        if (pauseExitBtn)
            pauseExitBtn.addEventListener('click', () => this.goToMenu());
        ...
        if (canvas) {
            ...
            canvas.addEventListener('mousedown', e => { if (e.button === 0)
                this.inputBoost = true; });
            ...
            const isButtonTarget = (e) => !!(e.target && e.target.closest && e.target.closest('button'));
            wrap.addEventListener('touchstart', e => {
                const t = e.touches[0];
                if (!t || isButtonTarget(e))
                    return;
                touchActive = true;
        ...
        if (boostBtn) {
            const on = (e) => { if (e)
                e.preventDefault(); this.inputBoost = true; boostBtn.classList.add('pressed'); };
            const off = (e) => { if (e)
                e.preventDefault(); this.inputBoost = false; boostBtn.classList.remove('pressed'); };
        ...
        document.addEventListener('keydown', e => {
            if (!isScreenVisible('snakeGameScreen'))
                return;
