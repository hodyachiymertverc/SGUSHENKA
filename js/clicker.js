...
const Clicker = {
  ...
  syncRecord(){
    ...
    const earned = Math.floor(this.data.totalEarned || 0);
    // счёт 0 не отправляем: (1) нет смысла засорять таблицу лидеров
    // записями с 0 банок у только что зашедших игроков; (2) это же
    // защищает от гонки при удалении игрока из админки ("Удалить
    // полностью") — сразу после удаления баланс обнуляется, но пока
    // игрок ещё на сайте, этот же таймер тикает каждые 4с и МОГ заново
    // создать в таблице рекордов запись с его старым именем прямо
    // следом за удалением — теперь при обнулённом счёте запись просто
    // не отправляется, и "призрак" удалённого игрока там не появится
    if(earned <= 0) return;
    DB.addRecordIn('clickerRecords', this.playerId, getNickname(), earned);
  },
  openRecords(){
